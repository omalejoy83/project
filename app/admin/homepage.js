...
class Home extends React.Component{
        ...
        componentWillUnmount(){
         ...
         _imageswitch(){
          const routeName = this.props.navigation.state.params? this.props.navigation.state.params.channelName:'Personal'
        ...
        _listening(){
          const id = this.props.navigation.state.params? this.props.navigation.state.params.channelId:'1MFsXrZfxvvhQtFqvAIF'
          const routeName = this.props.navigation.state.params? this.props.navigation.state.params.channelName:'Personal'
            ...
            if(routeName){
              ...
              this.listener = firestore.collection('Admin').doc(id)
              .collection(this.props.navigation.state.params.userId).orderBy('createdAt', 'DESC')
              .onSnapshot((snapshot)=>{
       ...
       //1MFsXrZfxvvhQtFqvAIF Personal 
        ...
        ReceivedText(message=[]){
          ...
          const id = this.props.navigation.state.params? this.props.navigation.state.params.channelId:'1MFsXrZfxvvhQtFqvAIF'
         firestore.collection('students').doc(this.props.navigation.state.params.userId)
          .collection(id).add(message[0])
          firestore.collection('Admin').doc(id)
        .collection(this.props.navigation.state.params.userId).add(message[0])
        }
      ...
      render(){
        //const {userName,password} = this.props.navigation.state.params
             ...
             <Body><Title>{this.state.routeName}</Title></Body>
                    <Right>
                    <Button iconleft icon={SearchIcon} appearance='ghost'></Button>
                    <Button onPress={()=> this._navigate('Settings')}  iconleft icon={MoreIcon} appearance='ghost' ></Button>
                    </Right>
                </Header>
