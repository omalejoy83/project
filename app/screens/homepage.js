...
class Home extends React.Component{
        ...
        _listening(){
          ...
          const routeName = this.props.navigation.state.params? this.props.navigation.state.params.data.name:'Personal'
            ...
            if(routeName){
              ...
              this.listener = firestore.collection('students').doc(auth.currentUser.uid)
              .collection(id).orderBy('createdAt', 'DESC')
              .onSnapshot((snapshot)=>{
        ...
        ReceivedText(message=[]){
         ...
         .collection(auth.currentUser.uid).add(message[0]);
          firestore.collection(id).doc(auth.currentUser.uid).set({id:auth.currentUser.uid,date:new Date(),name:auth.currentUser.displayName,photoUrl:'',email:auth.currentUser.email})
        }
